  ...
  useEffect(() => {
    ...
    const relire = async () => {
      const { data } = await lectureCachee("clients", () =>
        supabase
          .from("clients")
          .select("id, nom, nom_facture, adresse, nif, stat, refs_legales, telephone_fac, langue, modele, coordonnees_paiement, dates_oeufs, montant_lettres, rib_pied, afficher_conditions, conditionnement, conditionnements, delai_paiement_jours, tarifs_clients(calibre, prix, prix_facture)")
          .eq("actif", true)
      );
      if (!vivant) return;
      if (data?.length) {
